...
const configFilePath = path.join(__dirname, '../try-bot-431201-5a31739dd1a9.json');

function loadBlastConfig() {
  try {
    const blastFilePath = path.join(__dirname, '../config/blast.json');
    console.log('Mencoba membuka file:', blastFilePath);

    const blastConfig = JSON.parse(fs.readFileSync(blastFilePath, 'utf-8'));
    console.log('Isi blastConfig:', blastConfig);
    return blastConfig;
  } catch (error) {
    console.error('Error membaca file blast.json:', error);
    return { "no tujuan": [], "pesan": "" };
  }
}

let blast = loadBlastConfig();
